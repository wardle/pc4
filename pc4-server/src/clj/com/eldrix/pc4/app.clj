...
(defn patient-menu
  [ctx {:t_patient/keys [id patient_identifier]} {:keys [selected-id]}]
  (let [router (get-in ctx [:request ::r/router])]
    (ui.misc/vertical-navigation
     {:selected-id selected-id
      :items [{:id :demographics
               :icon (ui.misc/icon-user)
               :content "Demographics"}
              {:id :nhs
               :icon (ui.misc/icon-building-office)
               :content "NHS"}
              {:id :research
               :icon (ui.misc/icon-building-library)
               :content "Research"}
              {:id :referrals
               :icon (ui.misc/icon-envelope-open)
               :content "Referrals"}]})))
...
(defn sanitise-redirect-url
  ...
  relative URLs on the same host, so strip out additional information such as
  host and port. This could insted use a regular expression to check for
  approved redirects."
  [url]
...
(def login
  "Logic for application login. This is currently only designed for users
   ...
   of a value for :user. Alternatively, the redirect URL could be signed in a
   token so we know it was generated by pc4."
  {:enter
   (fn [{request :request,
     ...
     (let [router (::r/router request)
           ...
           url (sanitise-redirect-url (get-in request [:params "url"]))
