...
(defn ^:export init []
  ...
  (reset! SPA (make-SPA))
  (ido (it/add-fulcro-inspect! @SPA))
  (app/set-root! @SPA root/Root {:initialize-state? true})
  (dr/initialize! @SPA)
  (pc4.route/start!)
  (app/mount! @SPA root/Root "app" {:initialize-state? false})
  ;; Check for existing session - load authenticated user from server
  (df/load! @SPA :session/authenticated-user pc4.ui.users/UserHomePage
            {:post-mutation `pc4.users/mark-session-checked
             :fallback      `pc4.users/mark-session-checked}))
