...
(defsc UserHomePage
  [this {:>/keys      [projects]
         :t_user/keys [id active_roles latest_news must_change_password]
         :as props}]
  {:ident         :t_user/id
   :query         [:t_user/id :t_user/username :t_user/title
                   ...
                   {[:component/id :patient-search] (comp/get-query search/PatientSearchResults)}
                   {:t_user/latest_news (comp/get-query NewsItem)}]
   :initial-state {:>/projects         []
                   :t_user/latest_news []}}
  (let [patient-search (get props [:component/id :patient-search])
        search-performed? (:ui/search-performed? patient-search)
        project-ids (keys active_roles)]
    (div :.flex.flex-col.md:flex-row.md:gap-4.m-4
         ;; Left column: search box + projects (fixed width on desktop)
         (div :.w-full.md:w-64.md:shrink-0.space-y-4
              ;; Search box - pass project-ids for "my patients" filtering
              (div :.mb-4
                   (search/ui-patient-search-box (comp/computed patient-search {:project-ids project-ids})))
              ;; Projects list
              (ui-list-user-projects projects))
         ;; Right column: search results OR news (fills remaining space)
              ...
              (when must_change_password
                (ui/ui-panel {:classes ["bg-red-50" "text-red-800"]}
                             ...
                             (ui/ui-button {:role    :primary
                                           ...
                                           "Change password...")))
              ;; Show search results if search performed, otherwise show news
              (if search-performed?
                (search/ui-patient-search-results patient-search)
