...
(defsc SummaryMultipleSclerosis
  [this {:t_summary_multiple_sclerosis/keys [id ms_diagnosis_fk events event_ordering_errors]
         :ui/keys                           [current-patient all-ms-diagnoses] :as params}]
  ...
  (let [patient-identifier (:t_patient/patient_identifier current-patient)
        not-ms-diagnosis (first (filter #(= 15 (:t_ms_diagnosis/id %)) all-ms-diagnoses))
        ms_diagnosis (first (filter #(= ms_diagnosis_fk (:t_ms_diagnosis/id %)) all-ms-diagnoses))]
    (tap> {:sms params})
    (js/console.log "sms id:" id " ms diagnosis:" ms_diagnosis_fk)
    (comp/fragment
      (ui/ui-panel {:classes ["mb-4"]}
        (div :.flex.flex-col.sm:flex-row.sm:items-center.sm:gap-4
          (dom/label :.block.text-sm.font-medium.text-gray-700 "Neuroinflammatory diagnostic category")
          (div :.pt-2.sm:pt-0
            (ui/ui-select-popup-button
              {:value         (or ms_diagnosis not-ms-diagnosis)
               :default-value not-ms-diagnosis
               :options       all-ms-diagnoses
               :id-key        :t_ms_diagnosis/id
               :display-key   :t_ms_diagnosis/name
               :onChange      #(comp/transact! this [(list 'pc4.rsdb/save-ms-diagnosis (assoc % :t_patient/patient_identifier patient-identifier))])}))))
      ...
      (ui/ui-table {}
        ...
        (ui/ui-table-body {}
          (for [event (reverse (sort-by #(some-> % :t_ms_event/date .valueOf) events))]
...
(defsc PatientNeuroInflammatory
  [this {:t_patient/keys [id patient_identifier summary_multiple_sclerosis]
         :>/keys         [layout], :ui/keys [editing-ms-event]}]
  ...
  (let [has-summary? (:t_summary_multiple_sclerosis/id summary_multiple_sclerosis)
        show-edss-chart? (comp/get-state this :show-edss-chart?)]
    (when patient_identifier
      (let [do-add #(comp/transact! this [(add-ms-event {:patient-identifier patient_identifier
            ...
            do-show-chart #(comp/set-state! this {:show-edss-chart? true})
            do-hide-chart #(comp/set-state! this {:show-edss-chart? false})]
        (patients/ui-layout layout
          {:selected-id :neuroinflammatory
           :sub-menu    [{:id      :add-ms-event
                          :onClick do-add
                          :hidden  (not has-summary?)
                          :content "Add disease event..."}
                         {:id      :edss-chart
                          :onClick do-show-chart
                          :hidden  (not has-summary?)
                          :content "EDSS chart"}]}
          (comp/fragment
            (when (:t_ms_event/id editing-ms-event)
              (ui-edit-ms-event editing-ms-event))
            (when show-edss-chart?
              (ui/ui-modal
                {:onClose do-hide-chart
                 :size    :xl
                 :actions [{:id :close :title "Close" :onClick do-hide-chart}]}
                (dom/img {:src (str "/patient/" patient_identifier "/chart?type=edss&width=1200&height=600")})))
            (when has-summary?
