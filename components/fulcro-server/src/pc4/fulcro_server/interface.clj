(ns pc4.fulcro-server.interface
  (:require
   ...
   [pc4.fulcro-server.charts :as charts]
   [pc4.fulcro-server.graph :as graph]
...
(def authenticated
  "Interceptor that requires an authenticated user in the session."
  {:name ::authenticated
   :enter
   (fn [ctx]
     (if (get-in ctx [:request :session :authenticated-user])
       ctx
       (do
  ...
  {:name ::api
   ...
   (fn [{:keys [rsdb] :as ctx}]
     ...
     (let [params (get-in ctx [:request :transit-params])
           ...
           authorization-manager (when authenticated-user
                                   (rsdb/authorization-manager authenticated-user))]
       (execute-pathom ctx {:request                      (:request ctx)
...
(def routes
  (route/expand-routes
   #{["/" :get [landing]]
     ["/login" :post [login]]
     ["/api" :post [authenticated api]]
     ["/users/:system/:value/photo" :get [authenticated get-user-photo]]
     ["/patient/:patient-identifier/chart" :get [authenticated charts/chart-handler]]}))
...
(s/def ::pathom (s/keys :req-un [::graph/ops ::graph/env]))
(s/def ::rsdb rsdb/valid-service?)
(s/def ::use-tailwind-cdn boolean?)
(s/def ::env (s/keys :req-un [::rsdb ::pathom]
                     :opt-un [::use-tailwind-cdn]))
(s/def ::cljs-manifest string?)
(s/def ::port pos-int?)
(s/def ::host string?)
(s/def ::join? boolean?)
(s/def ::allowed-origins string?)
...
(defmethod ig/init-key ::server
  ...
  (log/info "starting HTTP server" (select-keys config [:port :host :cljs-manifest]))
  ...
  (when (:use-tailwind-cdn env)
    (log/warn "using Tailwind CDN instead of local CSS"))
  (let [cljs-modules (find-cljs-modules cljs-manifest)
        pathom (graph/boundary-interface (:pathom env))
        env' (-> env (dissoc :pathom) (assoc :pathom pathom :cljs-modules cljs-modules))]
...
(comment
  ...
  (ig/load-namespaces (pc4.config.interface/config :dev))
  (require '[pc4.graph.interface])
  (require '[pc4.graph.rsdb])
