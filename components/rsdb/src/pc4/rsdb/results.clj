...
(s/fdef save-result*
  :args (s/cat :conn ::db/txn
               :result-type (s/keys :req-un [::entity-name ::table ::spec])
...
(s/fdef normalize-result
  :args (s/cat :result map? :result-type (s/keys :req-un [::entity-name ::table] :opt-un [::summary-fn])))
...
(s/fdef -results-from-table
  :args (s/cat :conn ::db/conn
               ...
               :result-type (s/keys :req-un [::entity-name ::table] :opt-un [::fetch-fn ::summary-fn])))
...
(defn mri-brains-for-patient
  ...
  (let [results (->> (fetch-mri-brain-results conn nil {:t_patient/patient_identifier patient-identifier})
...
(s/fdef save-mri-brain!
  :args (s/cat :conn ::db/txn
               :result-type (s/keys :req-un [::entity-name ::table])
