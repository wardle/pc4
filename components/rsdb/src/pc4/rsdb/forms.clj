...
(def form-type-by-id
  "Return form type by id (legacy integer form-type-id).
  e.g.,
  ```
    (form-type-by-id 2)
    => {:form-type-id 2, :table \"t_form_edss\", :title \"EDSS (short form)\", ...}
  ```"
  (reduce (fn [acc {:keys [form-type-id] :as form-type}]
            (if form-type-id
              (assoc acc form-type-id form-type)
              acc))
          {} forms))
