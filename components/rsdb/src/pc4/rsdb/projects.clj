...
(defn ^:deprecated project-id-for-admission
  "DEPRECATED: should be a first class function here to save an admission."
  [conn])

(defn save-admission!
  [conn user-id {:t_episode/keys [project_fk patient_fk date_referral date_registration date_discharge] :as episode}]
  (let [project-id (or project_fk
        ...
        episode' (merge ;; choose some sane defaults, but allow override
    ...
    (if (tempid/tempid? (:t_episode/id episode'))
      ...
      (next.jdbc.sql/update! conn :t_episode episode' {:id (:t_episode/id episode')} {:return-keys true}))))
...
(def patient-by-project-pseudonym search-by-project-pseudonym)
